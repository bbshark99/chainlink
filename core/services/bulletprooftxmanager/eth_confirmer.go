...
func NewEthConfirmer(db *gorm.DB, ethClient eth.Client, config Config, keystore KeyStore, advisoryLocker postgres.AdvisoryLocker, keys []ethkey.Key, estimator gas.Estimator, resumeCallback func(id uuid.UUID, value interface{}) error) *EthConfirmer {
...
func (ec *EthConfirmer) processHead(ctx context.Context, head models.Head) error {
	...
	if ec.resumeCallback != nil {
		...
		logger.Debugw("EthConfirmer: finished ResumePendingTaskRuns", "headNum", head.Number, "time", time.Since(mark), "id", "eth_confirmer")
	}
