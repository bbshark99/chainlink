...
func TestBulletproofTxManager_CreateEthTransaction_OutOfEth(t *testing.T) {
	...
	thisKey, _ := cltest.MustInsertRandomKey(t, db, etKeyStore, 1)
	otherKey, _ := cltest.MustInsertRandomKey(t, db, etKeyStore, 1)
...
func TestBulletproofTxManager_Lifecycle(t *testing.T) {
	...
	config.On("GasEstimatorMode").Return("FixedPrice")
	kst.On("AllKeys").Return([]ethkey.KeyV2{}, nil).Once()
	...
	kst.On("AllKeys").Return([]ethkey.KeyV2{key}, nil).Once()
